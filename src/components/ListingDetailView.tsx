...
export default function ListingDetailView({ asModal = false }: { asModal?: boolean }) {
  const { id } = useParams();
  ...
  // Swipe-down-to-dismiss: drag the modal/page down and release to go back.
  // When `asModal` is true, the gesture drives the fixed overlay container and
  // reveals whatever is behind it (intercepting route → home page stays mounted).
  ...
  // Modal mount animation — start offscreen (bottom), slide up on mount
  useEffect(() => {
    if (!asModal) return;
    // Start at translateY(100%) on first paint, then flip to 0 next frame so the
    // transition actually animates. requestAnimationFrame ensures we paint the
    // initial state before changing it.
  ...
  // - Page mode: attached to the document, uses window.scrollY / window.scrollTo.
  // - Modal mode: attached to the modal's scroll container, and the drag moves
  //   the entire modal overlay, revealing whatever is behind (home page from
  //   the parallel children slot — that's the Facebook-style animation).
  useEffect(() => {
    ...
    const scrollEl = asModal ? scrollRef.current : null;
    if (asModal && !scrollEl) return; // wait for ref

    const getScrollTop = () => (asModal ? scrollEl!.scrollTop : window.scrollY);

    const isInsideIgnore = (target: EventTarget | null) => {
    ...
    const onTouchStart = (e: TouchEvent) => {
      ...
      if (isInsideIgnore(e.target)) return;
      // Only arm the gesture when already scrolled to the top
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      // Downward drag from the top — take over
    ...
    const target: HTMLElement | Document = asModal ? scrollEl! : document;
  ...
  // Wrap content in modal chrome when asModal is true. The modal container is a
  // full-viewport fixed overlay with its own scroll. Its transform is driven by
  // the drag state so swipe-down slides the entire modal out revealing the home
  // page that stays mounted in the parallel `children` slot beneath.
  const wrap = (inner: React.ReactNode) => {
    if (!asModal) return inner;
    const translateY = !mounted ? "100%" : `${dragY}px`;
    const transitionTransform =
      !mounted || dragY === 0 || isDismissing ? "transform 300ms cubic-bezier(0.22, 0.61, 0.36, 1)" : "none";
    return (
      <div className="fixed inset-0 z-[60]" aria-modal="true" role="dialog">
        {/* Backdrop — fades out as the modal is dragged down, revealing the page behind */}
        <div
          className="absolute inset-0 bg-black pointer-events-none"
          style={{
            opacity: !mounted ? 0 : Math.max(0, 0.45 - dragY / 1200),
            transition:
              !mounted || dragY === 0 || isDismissing ? "opacity 300ms cubic-bezier(0.22, 0.61, 0.36, 1)" : "none",
          }}
        />
        {/* Scrollable modal surface */}
        <div
          ref={scrollRef}
          className="absolute inset-0 overflow-y-auto overflow-x-hidden overscroll-contain bg-surface-secondary shadow-2xl"
          style={{
        ...
        >
          ...
          <button
            ...
            className="fixed top-4 left-4 z-10 w-10 h-10 bg-white/95 backdrop-blur-sm rounded-full flex items-center justify-center shadow-md hover:bg-white transition"
            aria-label="Close"
  ...
  // Page-mode-only backdrop (kept for non-modal direct navigation). In modal
  // mode the wrap() helper owns the backdrop and container transform.
  const pageModeBackdrop = !asModal && (
    <div
  ...
  const pageModeTransform = !asModal
    ? {
